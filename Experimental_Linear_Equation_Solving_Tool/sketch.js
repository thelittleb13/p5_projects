...
// use KaTeX, math.js/fraction.js to do fractions!
...

{
  ...
  function createColorDict() {
    for (var i = 0; i < terms[step].length; i++) {
      let variable = terms[step][i].variable;
      if (!arrayOfVariables.has(variable)) arrayOfVariables.push(variable);
    }

    for (let variable of arrayOfVariables) {
      if (colorDict[variable] == undefined)
        colorDict[variable] = getRandomColor();
    }
  }

  Array.prototype.has = function (element) {
    var doesElementExist = false;
    for (let i = 0; i < this.length; i++) {
      if (this[i] === element) doesElementExist = true;
    }
